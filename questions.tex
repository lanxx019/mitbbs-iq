...
\begin{Q}[Amazon]{http://www.mitbbs.com/article_t/JobHunting/32248787.html}
给一个文件，里面有name，address, zip code,每行一个数据。格式什么的没问题，都是comma separted， 问怎么列出来所有的zip code，结果不能有重复。
\end{Q}

\begin{Q}[Amazon]{http://www.mitbbs.com/article_t/JobHunting/32259945.html}
Implement LRU(least recently used) cache.
\end{Q}

\begin{Q}[Amazon]{http://www.mitbbs.com/article_t/JobHunting/32259945.html}
1-100的整数，有一个数出现两次，其余的数出现一次。找到出现两次的那个数。
\end{Q}

\begin{Q}[Amazon]{http://www.mitbbs.com/article_t/JobHunting/32271487.html}
给一个Set of word, 查找某一个词是否在这个set里。
\end{Q}
...
\begin{Q}[Amazon]{http://www.mitbbs.com/article_t/JobHunting/32282115.html}
OO design来表示formula，比如$1+2*3$. 要求有\ilcode{calculate()}和\ilcode{tostring()}函数。
\end{Q}

\begin{Q}[Amazon]{http://www.mitbbs.com/article_t1/JobHunting/32292525_0_1.html}
...
\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32026405.html}
有个封装好的函数\ilcode{int blockRead(char[] buf)}。内部有个静态文件指针，只能向文件末尾移动，不能rewind每次只能读取4K的block到buf里，返回读取的字节数（除非到文件尾，否则总是4K）要求实现\ilcode{int anySizeRead(char[] buf, int size)}从文件的当前位置读取任意大小的数据存入buf，并返回实际读到的数据字节数
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32026405.html}
Skip list, http://en.wikipedia.org/wiki/Skip\_list。写code实现。
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32026405.html}
a set of intervals, how to tell if a given value is in any of the intervals
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32060089.html}
写一个程序，模拟投硬币的过程，每次投硬币，直到出现正面为止，返回之前出现反面的次数。重复N次上述的过程，问一共出现多少次反面
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32064283.html}
$10^9$个star，亮度为double类型，找出前一千个最亮的
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32067247.html}
给三个数组，怎么穷举所有的组合。。。比如$arr1=[a,b]$, $arr2=[3,4,5]$, $arr3=[true, false]$。如果不是三个，不确定个数的数组怎么穷举。
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32079701.html}
Design and implement a class to train from a input string like ``ape apple ace'' to generate new words based on conditional probability of $P(c_i|c_j)$. e.g., $P(p|a)$, $P(e|p)$, $P(l|p)$.

\begin{lstlisting}
class TokenGenerator {
	public void train(String t) {\ldots}
	public String generate() {\ldots}
}
\end{lstlisting}

How to optimize \ilcode{generate()} method.
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32107851.html}
两个BST，求他们merge后的BST。
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32128895.html}
有一個map的interface，要實現\ilcode{get()}, \ilcode{put()}, \ilcode{remove()}, \ilcode{getRandom()}。
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32119891.html}
About Inheritance and polymiorphism. A class, like LinkedList in Java, has two methods \ilcode{add()} and \ilcode{addall()}. Write a subclass to count how many times \ilcode{add()} is called. 
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32175265.html}
一个游戏，叫做“生或死” a.k.a. Game of Life，在一个棋盘上，规则如下：每格有两种状态：生，或者死
...
\begin{Q}[Google]{http://www.mitbbs.com/article_t1/JobHunting/32195653_0_1.html}
写一个类是timer的东西， 例如给个数值t和函数，等t时间之后call 这个函数。（然后问有多个这些如果支持多次调用怎么办，有哪些问题之类的）
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t1/JobHunting/32195653_0_1.html}
二个人轮流打枪的问题算概率， 就是6发装弹夹里面有一颗子弹。然后轮流对照自己头打，然后在shuffle对方接着打。
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t1/JobHunting/32195653_0_1.html}
写个函数输入7张牌，然后输出是否有同花顺，顺子，和同花。 
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t1/JobHunting/32195653_0_1.html}
一个billion of urls, 然后让你输出最长的相同的prefix，包含这个prefix url必须占$75\%$以上。
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32224635.html}
假设有一很长的数字，比如11224444,被压缩存储为pairs，如(1, 2), (2, 2), (4, 4).然后让我写个Iterator，要求有\ilcode{constructor()}，\ilcode{next()} to give out the digit and move to the next position, \ilcode{hasNext()} to indicate if there's any more digit left.
\end{Q}
...
\begin{Q}[ZocDoc]{http://www.mitbbs.com/article_t1/JobHunting/32343179_0_2.html}
给一个array，要求判断存不存在一个subarray的和是0。subarray的长度不限。要求$O(n)$
\end{Q}

\begin{Q}[Google]{}
设计一个方法来encode和decode 32 bit integer。要求能节省空间。比如：

如果int是1，可以用1个byte

如果int是256，可以用2个bytes
\end{Q}

\begin{Q}[Twitter]{http://www.mitbbs.com/article_t/JobHunting/32361483.html}
给一个字典，里面有一堆词。然后给一个词，word，求返回，与word编辑距离<=1的所有出现在字典里的词。
\end{Q}

\begin{Q}[Facebook]{http://www.mitbbs.com/article_t0/JobHunting/32364859.html}
...
\begin{Q}[Microsoft]{http://www.mitbbs.com/article_t/JobHunting/32369055.html}
一个人与人之间认识的关系网，单向的，就是我认识你，你不一定认识我。每两个人之间至少有一种认识关系。如果一个人被别人认识，但都不认识别人，叫做celebrity。问是否存在这样的celebrity，如果存在，可否存在多于一个的celebrity。然后问怎么去represent这样的关系
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32370913.html}
有一个只能产生0和1的随机函数，写一个能产生0到n-1的随机函数
\end{Q}

\begin{Q}[Apple]{http://www.mitbbs.com/article_t/JobHunting/32371887.html}
Find the lonely celebrity(celebrity: everyone knows him, he knows no one). Given a function \ilcode{boolean knows(A, B)}, which tells you if A knows B. 

Implement function: \ilcode{List<String> getLonelyCelebrity(Set<String> names)}.
\end{Q}
...
\begin{Q}[Rocket Fuel]{http://www.mitbbs.com/article_t/JobHunting/32402829.html}
...
\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32439261.html}
Write code to compute the shortest unique prefix of each word in a set of words.
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32449259.html}
给定二维网格里N个点的坐标，取其中任意一个点，找到所有距离这个点不超过K的坐标点。距离的定义是X轴距离+Y轴距离。
\end{Q}

\begin{Q}[Twitter]{http://www.mitbbs.com/article_t/JobHunting/32436737.html}
括号匹配，给定字符串，输出括号是否匹配.要求必须用递归写.
\end{Q}

\begin{Q}[Twitter]{http://www.mitbbs.com/article_t/JobHunting/32436737.html}
最长连续上升子串，给定字符串，输出最长连续上升子串的起始点和长度.
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32455493.html}
回到1980年代，一台计算机内存1KB, 主频 1MHz, 设计一个在这台机器上跑的时间最长(但能确定会正常结束)的程序，证明为什么是最长，并计算时长。
\end{Q}

\begin{Q}[10gen]{http://www.mitbbs.com/article_t/JobHunting/32455661.html}
给一个interface：
...
Stream都是infinite的。getNext()永远return一个比上次return的值还要大或者相等的int。让你实现一个class：
...
要求这个class在constructor里面accept一个array的Stream，然后同时实现getNext() method，也要永远返回比上一次大或者相等的int。最后问了如果有N个Stream，然后getNext被用了K次的话，复杂度是个什么情况。
\end{Q}

\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32184907.html}
...
\begin{Q}[Google]{http://www.mitbbs.com/article_t/JobHunting/32466847.html}
A binary search tree is given. Find the ceiling value present in the BST of a given key. eg:
...
\begin{Q}[Facebook]{http://www.mitbbs.com/article_t/JobHunting/32469031.html}
given an array, find out the max sum of a set that each elements are non-ajacent.
\end{Q}

\begin{Q}[Facebook]{http://www.mitbbs.com/article_t/JobHunting/32429075.html}
给一堆F的用户，以及朋友关系，朋友之间的关系是双向的。问能否将朋友的关系图分成两个partition。使得任何有直接朋友关系的两个人必须处在不同的partition里。
\end{Q}

\begin{Q}[Facebook]{http://www.mitbbs.com/article_t/JobHunting/32429075.html}
三个数组，从三个数组各取一数，找出和为某个值的组合
\end{Q}

\begin{Q}[Facebook]{http://www.mitbbs.com/article_t/JobHunting/32429075.html}
平面上一堆点，找出四个点，使得四边形面积最大。问题简化成找三个点，使得三角形面积最大。
\end{Q}

\begin{Q}[Pinterest]{http://www.mitbbs.com/article_t/JobHunting/32295631.html}
...
\ilcode{canJump(int a[], int pos)}
\end{Q}

\begin{Q}[Facebook]{http://www.mitbbs.com/article_t/JobHunting/32468029.html}
一个链表1->2->3->4->5转换成1->5->2->4->3
\end{Q}

\begin{Q}[Yelp]{http://www.mitbbs.com/article_t/JobHunting/32466727.html}
给定一些字母和一个字典，找这些字母能组成的所有的词,如何scale
\end{Q}
...
\begin{Q}[Google]{}
如何产生一个随机的包含N个节点的binary tree?
\end{Q}

\begin{Q}[Google]{}
给一组字符串，找出一个字符串对儿，使得$a.size() * b.size()$最大，其中$a$和$b$必须没有相同的字母.
\end{Q}

\begin{Q}[]{}
给定$n$个实数$x_1, x_2, x_3....x_n$，求这$n$个实数在实轴上向量$2$个数之间的最大差值，要求线性的时间算法。
\end{Q}
